...
- SVG河川レイヤー（青実線／橋は青破線）
- 橋は専用の編集UIではなく、**道路ヘックスサイドが河川ヘックスサイドと一致する地点を自動的に橋とみなす**方式（`isBridgeHexside`）。現在のデータでは1613-1614の1箇所のみ該当（Plancenoit付近、地図上で道路と河川が実際に交差している地点を目視確認済み）。他の河川区間は道路が並走するのみで交差しておらず、橋が存在しない状態
...
なし（移動済みフラグ・道路+坂道コストは修正済み。`acted`フラグと`isOnRoad`対応の`computeReachable`を参照）。
...
| 項目 | 備考 |
|------|------|
| 橋の要検証 | 道路×河川の自動交差判定で橋を出しているが、現データでは1箇所（1613-1614）のみ検出。Vassalモジュール等の実物資料で本来の橋の位置と数を確認し、必要なら道路データを補正する必要あり |
